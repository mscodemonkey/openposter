...
test("my media can apply OpenPoster movie logo artwork", async ({ page }) => {
  const fixtureCreatorDisplayName = "E2E Movie Logo";
  const theme = await ensureDefaultTheme();

  const uploaded = await uploadPoster({
    filePath: TED_POSTER_IMAGE,
    fileName: "dr-no-my-media-logo.jpg",
    mediaType: "movie",
    tmdbId: 646,
    title: "Dr. No",
    year: 1962,
  ...
  await logoCard.getByRole("button", {
  ...
  if (!applyResponse.ok()) {
    ...
    throw new Error(`Movie logo apply failed with ${applyResponse.status()}: ${responseText}`);
  }
  ...
  if (!applyRequestBody?.poster_id || !applyRequestBody?.asset_hash) {
    throw new Error(`Movie logo apply omitted tracking fields: ${JSON.stringify(applyRequestBody)}`);
  }

  const applyJson = (await applyResponse.json()) as { media_item_id?: string };
  const trackedMediaItemId = `${applyJson.media_item_id ?? movie!.id}:logo`;
  ...
  await expect
    .poll(async () => {
      ...
      timeout: 15_000,
      message: "expected the applied My Media movie logo artwork to be tracked on the node",
    })
