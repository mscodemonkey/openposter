...
test("my media can apply OpenPoster movie square artwork", async ({ page }) => {
  const fixtureCreatorDisplayName = "E2E Movie Square";
  const theme = await ensureDefaultTheme();

  const uploaded = await uploadPoster({
    filePath: TED_POSTER_IMAGE,
    fileName: "dr-no-my-media-square.jpg",
    mediaType: "movie",
    tmdbId: 646,
    title: "Dr. No",
    year: 1962,
  ...
  await squareCard.getByRole("button", {
  ...
  if (!applyResponse.ok()) {
    ...
    throw new Error(`Movie square apply failed with ${applyResponse.status()}: ${responseText}`);
  }
  ...
  if (!applyRequestBody?.poster_id || !applyRequestBody?.asset_hash) {
    throw new Error(`Movie square apply omitted tracking fields: ${JSON.stringify(applyRequestBody)}`);
  }

  const applyJson = (await applyResponse.json()) as { media_item_id?: string };
  const trackedMediaItemId = `${applyJson.media_item_id ?? movie!.id}:square`;
  ...
  await expect
    .poll(async () => {
      ...
      timeout: 15_000,
      message: "expected the applied My Media movie square artwork to be tracked on the node",
    })
