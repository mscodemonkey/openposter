interface PlexLogoProps {
  /** Height in px — width is calculated from the logo's aspect ratio. */
  height?: number;
}

/**
 * Full Plex wordmark (P L E + diamond X mark).
 * All paths use currentColor so they adapt to dark/light themes;
 * the X diamond base is always Plex amber.
 */
export default function PlexLogo({ height = 20 }: PlexLogoProps) {
  const width = Math.round(height * (1000 / 460.897));
  return (
    <svg
