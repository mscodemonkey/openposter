  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        // For episodes, fetch show title + backdrop from the tv_boxset endpoint
        if (p.media.type === "episode" && p.media.show_tmdb_id != null) {
          void fetch(`${base}/v1/tv_boxset/${encodeURIComponent(String(p.media.show_tmdb_id))}`)
            .then((r) => r.ok ? r.json() : null)
            .then((d: { show?: Array<{ media?: { title?: string }; assets?: { full?: { url?: string } } }>; backdrops?: Array<{ assets?: { full?: { url?: string } } }> } | null) => {
              const title = d?.show?.[0]?.media?.title;
              if (title) setTvShowTitle(title);
              const bUrl = d?.backdrops?.[0]?.assets?.full?.url ?? d?.show?.[0]?.assets?.full?.url ?? null;
              if (bUrl) setBackdropUrl(bUrl);
            })
            .catch(() => undefined);
        }
        ...
        if (p.media.tmdb_id && (p.media.type === "movie" || p.media.type === "show" || p.media.type === "season" || p.media.type === "episode")) {
          const u = new URL(`${base}/v1/search`);
          u.searchParams.set("tmdb_id", String(p.media.tmdb_id));
          u.searchParams.set("type", p.media.type);
          // Fetch more so we can filter by season client-side
          u.searchParams.set("limit", p.media.type === "episode" || p.media.type === "season" ? "50" : "12");
          const sr = await fetch(u.toString());
          if (sr.ok) {
            const sjson = (await sr.json()) as SearchResponse;
            let results = sjson.results.filter((x) => x.poster_id !== p.poster_id);
            // For episode/season, narrow to the same season
            if ((p.media.type === "episode" || p.media.type === "season") && p.media.season_number != null) {
              results = results.filter((x) => x.media.season_number === p.media.season_number);
            }
            results = results.slice(0, 20);
            setSimilarByTmdb(results);
            similarIds = new Set(results.map((x) => x.poster_id));
          }
        }
        ...
        if (p.creator.creator_id) {
          ...
          if (cr.ok) {
            ...
            setMoreByCreator(cjson.results.filter((x) => x.poster_id !== p.poster_id && !similarIds.has(x.poster_id)));
          }
        }
      } catch (e: unknown) {
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="space-between" alignItems={{ sm: "flex-end" }}>
          <Box sx={{ minWidth: 0 }}>
            <Typography variant="h4" sx={{ fontWeight: 900 }} noWrap>
              {poster?.media.type === "episode"
                ? `${tvShowTitle ?? "\u2026"}${poster.media.year ? ` (${poster.media.year})` : ""}`
                : `${poster?.media.title || t("posterLabel")}${poster?.media.year ? ` (${poster.media.year})` : ""}`}
            </Typography>
                ...
                {poster.media.type === "episode"
                  ? [
                      poster.media.season_number != null
                        ? `S${String(poster.media.season_number).padStart(2, "0")}`
                        : null,
                      poster.media.episode_number != null
                        ? `E${String(poster.media.episode_number).padStart(2, "0")}`
                        : null,
                    ]
                      .filter(Boolean)
                      .join(" · ")
          ...
          <>
            <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems="flex-start">
              <Box sx={{ width: { xs: "100%", md: "41.67%" }, flexShrink: 0 }}>
                <PosterCard
                  poster={poster}
                  showTitle={tvShowTitle ?? undefined}
                  showCreator={false}
                ...
                {(() => {
                  ...
                  return (
                    <Stack spacing={2}>
                      ...
                      {linksStatus && <Alert severity={linksStatus === t("linksSaved") ? "success" : "info"}>{linksStatus}</Alert>}
                    </Stack>
                  );
                })()}
              </Stack>
